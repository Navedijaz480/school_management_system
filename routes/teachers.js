...
const { sendEmailPassword, sendEmailToDEO , sendEmailAtReject} = require("../middleware/emailSender")
...
const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: 'stalliapp0@gmail.com',
        pass: 'your-email-password' // replace with your email password or app-specific password
    }
});

// Function to send email
async function sendEmail(to, subject, html, password) {
    const mailOptions = {
...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const html = `
            ...
            <p>Best regards,<br>School Administration</p>
        `;
        await sendEmailPassword(teacher.email, subject, html, password);
...
router.post('/abc', async (req, res) => {
    ...
    try {
        ...
        <p>Best regards,<br>School Administration</p>
        `;
        await sendEmailPassword(email, subject, html, password);
...
router.put('/:id', authenticateHeadmaster, verifyToken, async (req, res) => {
    const teacherData = req.body;
    const teacherAddress = teacherData.ethAddress; 
...
router.post('/requestChange', async (req, res) => {
    ...
    <p>Dear Administration,</p>
    <p>${req.body.name} has requested a transfer from the following school:</p>
    <p><strong>Current School:</strong> ${req.body.currentSchool}</p>
    <p><strong>Requested School:</strong> ${requestedSchool.split('|')[0]}</p>
    <p><strong>Reason for Transfer:</strong> ${req.body.reason}</p>
    <p>Please review the request and take necessary action.</p>
    <p>Best regards,<br>School Administration</p>
`;
        await sendEmailToDEO(subject, html);
...
router.post('/approveChange/:id', async (req, res) => {
    ...
    try {
        ...
        const teacherData = await Teacher.find().select('email');

        console.log(teacherData)

        for (const data of teacherData) {
        const subject = 'Transfer Request Accepted';
        const html = `
        <h1>Transfer Request Accepted</h1>
        <p>Dear ${teacher.name},</p>
        <p>We regret to inform you that your transfer request has been Accepted.</p>
        <p>If you have any questions, please contact the administration.</p>
        <p>Best regards,<br>School Administration</p>
        `;
        await sendEmailAtReject(data.email, subject, html);
        }
...
router.post('/rejectChange/:id', authenticateHeadmaster, async (req, res) => {
    ...
    try {
        ...
        <p>Best regards,<br>School Administration</p>
        `;
        await sendEmailAtReject(teacher.email, subject, html);
